...
--
SELECT TOP 20 rn = row_number() over (partition by wfna_m_id order by wfna_id asc), * FROM XXXX.dbo.tfi_billhead_a a --WHERE a.wfna_emp_id = '00000001' AND a.wfna_state = 0
--使用row_number（）函数进行编号
SELECT TOP 20 rn = row_number() over (order by wfna_id asc), * FROM XXXX.dbo.tfi_billhead_a a
